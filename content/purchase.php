<!--Purchase-->
<div id="purchase" class="col-xs-12">

	<div id="purchase-header" class="col-xs-12">
		<h1>Purchase</h1>
	</div>

	<div id="purchase-buttons" class="container">
		<ul>
			<li class="col-sm-push-3 col-sm-6 col-xs-12"><a class="btn btn-block btn-lg btn-default" href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=KAF3ZMR63QK56"><span class="glyphicon glyphicon-shopping-cart"></span> Pre-Order</a></li>
			<!--<li class="col-sm-6 col-xs-12"><a class="btn btn-block btn-lg btn-default">Ebook</a></li>-->
		</ul>
	</div>

	<div id="purchase-book" class="col-xs-6 col-xs-12">
		<img src="img/content/book.png">
	</div>

	<div id="purchase-ebook" class="col-xs-6 col-xs-12">
		<img src="img/content/ipad-book-coming-soon.png">
	</div>
	
</div>

<div id="purchase-footer" class="col-xs-12">
	<p id="purchase-footer-quote">&#8220;If you want to feel heard, understood, and more importantly set free from the wounds of divorce, you'll find your story intersecting with God's in the pages of <strong><em>LEFT</em></strong>.&#8221; <span id="purchase-footer-credit">- Ashley, 28</span></p>
</div>

<!--End Purchase-->